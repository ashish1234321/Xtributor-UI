...
const SLIDES = [
  {
    ...
    title: "Reselling Cloud the <span class='text-indigo-600'>old-fashioned way?</span> <br /> One nudge is all it takes to transform",
  ...
  },
  {
    ...
    title: "Launch your <span class='text-indigo-600'>online Marketplace</span> with hundreds of cloud products & services",
  ...
  },
  {
    ...
    title: "Automated provisioning, <span class='text-indigo-600'>tax tools</span>, pro-rata billing and instant payments",
  ...
  },
  {
    ...
    title: "Single login and <span class='text-indigo-600'>simple user interface</span> connecting to multiple Distributors",
    ...
    <section className="relative pt-32 pb-20 overflow-hidden group" id="hero">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="absolute top-1/2 -translate-y-1/2 left-4 xl:-left-16 z-20 hidden md:block">
          <button 
            onClick={handlePrev}
            className="p-4 rounded-full bg-white/80 hover:bg-white border border-slate-200 text-slate-400 hover:text-indigo-600 transition-all shadow-xl backdrop-blur-md group"
          >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <motion.div
              ...
              id="hero-content"
            >
              <div className="inline-flex items-center px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 text-xs font-bold uppercase tracking-widest mb-4 border border-indigo-100">
                {SLIDES[current].badge}
              </div>
              <h1 
                className="text-[27px] md:text-[33px] font-black text-slate-900 leading-[1.1] mb-4 tracking-tight"
              ...
              <div className="flex gap-2 mt-10">
                {SLIDES.map((_, i) => (
                  <button
                    ...
                    className={`h-1.5 transition-all duration-300 rounded-full ${i === current ? 'w-8 bg-indigo-500' : 'w-2 bg-slate-200 hover:bg-slate-300'}`}
                  />
                ))}
              </div>
            </motion.div>
